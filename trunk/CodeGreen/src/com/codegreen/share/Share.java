    	...
    	public void onComplete(Bundle values) {
            ...
            if (!values.containsKey("post_id"))
            {
                try
                {
                    ...
                    //parameters.putString("description", "RedEyeCTA");                 
                    
//                    parameters.putString("message", message);// the message to post to the wall
                    facebook.dialog(context, "stream.publish", parameters, this);// "stream.publish" is an API call
                }
                catch (Exception e)
                {
                    Toast.makeText(context, "Posted Error--" + e.getMessage(), Toast.LENGTH_LONG).show();
                }
            }
            else{
    	...
    	public void onError(DialogError arg0) {
    		Toast.makeText(context, "Dialog Error--" + arg0.getMessage(), Toast.LENGTH_LONG).show();
    		releaseFacebook();
    	}
...
//	    			Toast.makeText(context, "Wrong username/email or password", Toast.LENGTH_LONG).show();
	    			/*if (dialog!=null) {
	    				dialog.cancel();
	    			}*/
	    			...
	    			/*_doShareTwitter.cancel(true);
	    			Utility.showDialog(context, "", "Wrong user id/password.", 1);*/
	    		}
    		}
    	} catch (Exception e) {
...
//		emailIntent.putExtra(android.content.Intent.EXTRA_TEXT, "http://203.123.181.194/downloads/aat/test/re.htm");
...
//		Toast.makeText(context, "Message sent successfully.", Toast.LENGTH_LONG).show();
    ...
    public Handler screenHandler = new Handler() 
	{
		public void handleMessage(Message msg)
		{
			...
			{
			case TOASET_MSG:
				Toast.makeText(Share.context, "Please enter username/email and password", Toast.LENGTH_LONG).show();
				break; 
			case DISMISS: 
				outerDialog.dismiss();
				break;
